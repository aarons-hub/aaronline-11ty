(function () {
  ...
  var defaultCategory = categoryButtons[0]
  ...
  var state = {
    group:
      ...
      (partnerButtons[0]
        ? partnerButtons[0].dataset.group
        : thumbs[0]
          ? thumbs[0].dataset.group
          : ""),
  ...
  function getVisibleThumbs() {
    ...
    thumbs.forEach(function (thumb) {
      ...
      if (isServicesPage) {
        if (state.category === "Website") {
          isVisible = isTruthyFlag(thumb.dataset.featuredWeb);
        } else if (state.category === "Logo design") {
          isVisible = isTruthyFlag(thumb.dataset.featuredLogo);
        } else if (state.category === "Photography") {
          isVisible = isTruthyFlag(thumb.dataset.featuredPhoto);
        } else {
          isVisible =
  ...
  function syncView() {
    ...
    if (!visible.length && state.category !== defaultCategory) {
  ...
  var prevButton = page.querySelector("[data-thumb-prev]");
  var nextButton = page.querySelector("[data-thumb-next]");
  var scrollDistance = 360;

  if (prevButton && thumbStrip) {
    prevButton.addEventListener("click", function () {
      thumbStrip.scrollBy({ left: -scrollDistance, behavior: "smooth" });
    });
  }
